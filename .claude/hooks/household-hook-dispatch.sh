...
# a marker wins — an empty dir named open-claw-stuff must not satisfy it. The
# canonical repo is named `open-claw-stuff`, but the operator's primary Mac clones
# it as `~/ocs-work` (and `~/open-claw-stuff` does not exist there), so the real
# clone name is a recognized layout too — without it this dispatch resolved NOTHING
# on that machine and every leaf's HLS hooks were inert (verified 2026-09-05,
# slice of p1-measured-2026-08-13). open-claw-stuff is still tried FIRST.
ocs=""
...
if [ -z "$ocs" ]; then
  {
    ...
    echo "   Tried: \$HOUSEHOLD_OCS_ROOT, ${selfrepo}/../open-claw-stuff, \$HOME/open-claw-stuff, /workspace/open-claw-stuff, ${selfrepo}/../ocs-work, \$HOME/ocs-work"
