...
```bash
python3 /home/user/ken/orchestrator/memory_ops.py recall "session checkpoint" --domain allrecipes --limit 5
```
...
5. **On resume**, run `checkpoint.py scan` first, then recall the recovery memory:
   ...
   ```bash
   python3 /home/user/ken/orchestrator/memory_ops.py recall "rate limit recovery" --domain allrecipes --limit 3
   ```
